...
output_path = "/Users/bootkorea/Documents/GitHub/big_symbol/split_big_pnid/train"  # 결과를 저장할 디렉토리
...
def merge_directories(splitset_path, output_path):
    ...
    # Merge annfiles and images files from the train directory only
...
def apply_image_transform(image_path, txt_path, output_dir, transform_type):
    ...
    transformed_txt_path = os.path.join(output_dir, f"{os.path.splitext(os.path.basename(txt_path))[0]}_{transform_type}.txt")
    ...
    transformed_image_path = os.path.join(images_dir, f"{output_filename}_{transform_type}.png")
    cv2.imwrite(transformed_image_path, transformed_image)

merge_directories(splitset_path, output_path)

# 적용할 증강 기법 리스트
transform_types = ["vertical_flip", "horizontal_flip", "rotate_180"]
...
for txt_file in os.listdir(annfiles_dir):
    if txt_file.endswith(".txt"):
        ...
        for transform_type in transform_types:
            # 이미지 변환 및 좌표 수정 적용
            apply_image_transform(image_path, txt_path, annfiles_dir, transform_type)
